...
export function classify(err: XApiError): ErrorClass {
  ...
  if (err.type.includes('client-forbidden') && /duplicate/i.test(err.detail))
    return 'duplicate_content';
  if (err.type.includes('client-forbidden') && /not permitted/i.test(err.detail))
    return 'reply_restriction';
  if (err.type.includes('client-forbidden') && /user-suspended/i.test(err.detail))
