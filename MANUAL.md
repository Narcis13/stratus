...
5. **Composer** — single-post or **thread** mode (segment list, reorder, "link in tweet 1 = $0.20" warnings), live cost preview, "Move link to first reply", "Split into thread", "Suggest slot". Plus the **drafter**: pillar dropdown, idea seed (free text or Idea-Inbox dropdown), a **Tweet remix** box (seeded by the Voice tab's remix button, or paste any tweet — its shape is borrowed loosely, never its words) → 3 register-distinct drafts (plain/spicy/bait). The pillar dropdown defaults to **No pillar** — off-taxonomy drafting.
...
- **Original posts** — `POST /x/posts/draft` returns three register-distinct drafts (plain/spicy/bait), each declaring its pillar (or `none` when you draft off-pillar), grounded on your top-5 measured posts. ~$0.006–0.01/call.
