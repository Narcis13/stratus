...
>
> **Outliers — the X advanced-search compiler (2026-08-25):** **goals 3 + 4 — the intake funnel stopped depending on what the timeline happened to show me.** A new **Outliers** tab (Library rail, between Voice and Replies) compiles a structured form — keyword groups all/any/phrase/exclude, from/to/@/#, likes/reposts/replies floors, media/link/reply filters, language, date window — into a correct x.com advanced-search string, previews it live with a character count against a 512 cap, and hands it off **Copy first** (paste into X's own search box) with **Open in X** second; useful hunts are saved by name in `saved_searches` (migration `0031`) and re-loaded intact. **The whole feature is $0 by decision, not by luck, and it is invariant #8's opposite made on the record:** X API v2 has **no** `min_faves`/`min_retweets`/`min_replies` operator at any tier, so the API version would pay ~$0.005 per returned result to discard most of them — the web dialect supports them natively for nothing, `searchRecent` stays deleted, and nothing on any path here reaches `xFetch` or `askLLM`. **The results page needed no capture code at all**: `content.ts` attaches **Save to stratus** and the **⊕ Add to Radar** pin from one path-agnostic scan loop, so every search result already carries both. The intelligence is one dependency-free `src/shared/searchQuery.ts` inlined into the panel through a shim (the 12th), so the previewed string and the stored one cannot disagree — it owns the rules that are the feature (the OR group is **always** parenthesized because AND binds tighter on X; `OR` uppercase or it matches the literal word; a `"` in a term is an error since X has no escape character; `0` on a floor means *absent*, not *zero*; `lang:` is an allowlist because an unknown code returns nothing, which reads exactly like "no matches"; a query with only floors and filters is the firehose minus a bit of it, and errors). **Problems are a list with two severities and only one of them refuses** — errors disable Copy/Open and block a save, warnings (a `:` inside a keyword, a multi-word OR member) never block anything. Six `scope: 'server'` registry knobs (group **Outliers**, 63 → **69 knobs / 15 → 16 groups**, mirrored unmoved at 33) hold the defaults a fresh hunt opens with — **borrowed from a hand-verified cheat sheet rather than measured, which is exactly why they are knobs**; a computed threshold (timeline percentile / author median × k) is recorded as a separate plan and deliberately not built. The one measurement that did ship is the outcome: `voice_tweets.source = 'outlier_search'` (**the client reports which page, the SERVER decides what it means**, matched on the path *segment* so `/searchlight` is a profile — and first save wins, since provenance is something only the creating save can know) counted back as the tab's own footer. **Deliberately not built:** any API search path, a results scraper, an MCP tool, scheduled/recurring hunts. See `docs/outliers-tab.md`, `docs/PHASE-HISTORY.md` §"Outliers" and `plans/2026-08-03-search-query-builder.md`.

> **XR — the X ranker port (2026-09-02):** **goals 2 + 1 — a second number beside the coach's, computed with X's own published For You weights instead of with prose heuristics.** Two zero-dependency siblings carry it: `src/shared/xRanker.ts` transcribes the 26 production ranking heads and the `ranking_scorer.rs` arithmetic **from X's own repository** (`xai-org/x-algorithm` @ `7ba77684`, Apache-2.0; Bangermeter, MIT, is the prior art that inspired the feature and is credited in `xRankerSignals.ts`, which is what was read there — two files, two attributions, each citing what its author actually opened), and `src/shared/xRankerSignals.ts` maps **our** vocabulary onto them (`postCoach` check ids, `postFormat`, a small `DraftFeatures` struct) to produce **C** — prospective, over a draft's text — and **E** — retrospective, over a post's measured rates. Three surfaces: a `C 67 · typical` pill in the Composer's coach head (same debounced string, same coach result — two readings of one evaluation), an `E 52 · strong shape` chip on Radar sightings that scores **the post, never the drafted reply** (X drops out-of-network replies from For You outright), and an `E 47` badge leading the on-page action cluster so a hunt is judged before it is saved. **Migration `0032`** keeps the five numbers a save was made with on `voice_tweets`, as ONE observation — nullable throughout, because an absent count is unknown and never zero. **Every borrowed number was validated against our own corpus first and two of three checks changed what shipped:** `X_OBSERVED_RATES` is measured (n=766 of our own timeline rows; the retweet median is legitimately **0**), the borrowed 40/65 band cuts were a dead instrument — `below` fired on 0.3% of 308 own originals and `strong` on 0.1% of 3,567 sightings — and now ship as two measured quartile pairs, one per scale; the shrinkage constant K was measured and **kept**. **The falsification cell shipped and reads empty, which is an answer:** `rankerScoreEffectiveness` on `GET /x/playbook` cuts own harvested originals into quartiles by C and by E, and its population (`harvest_rows mode='posts'`, my own handle — never the frozen `metrics_snapshots`) has no rows yet, so C has no verdict. Advisory end to end: nothing sorts, gates, blocks or refuses on either number, and no panel renders the cell until it says something. Two silent `extractArticle` bugs fixed in passing (a quoted photo counted as the author's; a non-English locale zeroing every harvested metric). **$0 and structurally so** — no `xFetch`, no `askLLM`, no image call on any path; invariant #8 untouched. See `docs/PHASE-HISTORY.md` §"XR — the X ranker port", `docs/composer-tab.md` §"The ranker score (C)" and `plans/2026-09-02-x-ranker-port.md`.
