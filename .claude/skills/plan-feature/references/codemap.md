...
> **Stamped:** 2026-07-18 at commit `bc3f1e8` (ST.5). (Sha-stamp trap: ST.4's real commit is `15f2563` not `c1b0314`, ST.3's is `b82058b` not `292603d`, UI.10's is `c37d504` not `50948c9`, ST.2's is `dffd60a` not `860725f` — all amend-orphans, corrected here + in STATE.)
...
- Extension tests: bun:test on shared modules + studio (`templates.test.ts`, `milestones.test.ts` (S5.5 crossing logic), `brandKit.test.ts`, `mascot.test.ts`, `compose.test.ts`); DOM fixtures via happy-dom (earlyReplies); `sidepanel/storage.test.ts` mocks `chrome.storage.local` for the settings round-trip + `resolveTheme` (UI.9).
