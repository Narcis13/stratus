...
> **Stamped:** 2026-07-18 at commit `d8ecc9a` (RU.3 — `/replies/generate-batch` returns 3 angle variants per tweet; code-only, no migration). (Sha-stamp trap: RU.2's real commit is `182716a` not `ddecb5f`; RU.1's is `63df913` not `9c05464`; ST.9's is `d1902e5` not `47b10d2`, ST.8's `289ec2c` not `d38bbc5`, ST.7's `acd95ec` not `2104f46`, ST.6's `00b9615` not `b65266f`, ST.5's `1f6b20f` not `bc3f1e8`, ST.4's `15f2563` not `c1b0314`, ST.3's `b82058b` not `292603d`, UI.10's `c37d504` not `50948c9`, ST.2's `dffd60a` not `860725f` — all amend-orphans, corrected here + in STATE.)
...
Migrations `0000`–`0014` (latest: `0009` has_media, `0010` pinned_tweet_id, `0011` media_note, `0012` media_assets, `0013` app_settings, `0014` RU.2 reply `source` + radar `variants`/`model`/`reply_draft_id` — all nullable ADDs, DDL-only). Workflow: edit schema → `bun run db:generate` → **inspect the SQL** (drizzle drops seed INSERTs; keep them idempotent) → boot/`deploy.sh` migrates. (biome now ignores `src/db/migrations` — UI.1, D11/D13.)
