...
## WAVE 9 IS OPEN — the masterplan re-opened at 149/157. Next up: OU.1 or OU.3 (parallel).
...
- **Wave 9 — Outliers: X advanced-search compiler, clipboard hand-off, saved hunts (1/8)**: plan
  ...
  - [ ] **OU.1** pure compiler `src/shared/searchQuery.ts` + `FAVES_LADDER` — no deps, **xhigh**. Its x.com spot check is implementation, not formality.
...
**OU.1 (`src/shared/`) ∥ OU.3 (`src/x/settings/`) is the wave's only parallel pair** — fully disjoint trees, and
neither can start the other's file. Everything from OU.4 on is a single lane. The documented
