...
- **last-commit:** `c1b0314` (ST.4). **Sha reconciliation (see D16 / D17 / D14a / D15):** at ST.4 start STATE said `292603d` (ST.3) but HEAD was `b82058b` — `292603d` was another amend-orphan (sha-stamp trap), real ST.3 commit is **`b82058b`**; UI.10's recorded `50948c9` was ALSO an orphan → real **`c37d504`**. Both corrected in the ledger + codemap header + §11. Earlier same trap: ST.2 `860725f`→**`dffd60a`**, UI.9 `8c89950`→**`f9ff346`**; rewritten `7e07dd5`/`40c718e`/`95b9fff` → actual Wave-0 **UI.1 = `226368c`, UI.8 = `1299e43`, ST.1 = `61a04e7`** (D12). All ticked work verified present; only recorded shas were ever stale, no status drift.
- **current wave:** Wave 0 foundations COMPLETE. Lane C Studio: ST.1/ST.2/ST.3/**ST.4** done → **ST.5** (milestone+streak, dep ST.3+ST.4 ✓ now), **ST.6** (code card, dep ST.1+ST.2 ✓), **ST.7** (thread cover, dep ST.2+ST.3 ✓), **ST.8** (chart, dep ST.4 ✓ now) all eligible. Wave 1 (Lane A) still open.
- **next-up:** Lane C → **ST.5** (milestone+streak cards — also adds `metrics.account` api client + widens `ASSET_KINDS`), **ST.6** (code card + mono font), **ST.7** (thread cover — touches `Composer.tsx`/`StudioSeed`), or **ST.8** (chart card). All clean, no hot-file locks. Lane A → Wave 1 server (RU.1 / N.1 / AI.1 — all dep-free). **UI lane** still blocked until the Wave-1 settings-mirror chain (UI.6) lands.
...
- [x] ST.4 Patterns + presets (high) — c1b0314 2026-07-18
