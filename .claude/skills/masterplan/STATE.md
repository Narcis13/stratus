...
- **last-commit:** `9c05464` (RU.1). **Sha reconciliation (D23):** at RU.1 start STATE said `47b10d2` (ST.9) but that was an amend-orphan — real ST.9 = **`d1902e5`**; then **`906f2d3` ("eval 1")** was committed OUTSIDE the skill (the Wave-0 `src/db/client.ts` `:memory:`-under-test fix + a `stratus.db` backup, already RESOLVED in gotchas). Reconciled per Step-0; no status drift. Prior traps: ST.8 `d38bbc5`→**`289ec2c`**, ST.7 `2104f46`→**`acd95ec`**, ST.6 `b65266f`→**`00b9615`**, ST.5 `bc3f1e8`→**`1f6b20f`**, ST.4 `c1b0314`→**`15f2563`**, ST.3 `292603d`→**`b82058b`**, UI.10 `50948c9`→**`c37d504`**, ST.2 `860725f`→**`dffd60a`**, UI.9 `8c89950`→**`f9ff346`**; Wave-0 **UI.1 = `226368c`, UI.8 = `1299e43`, ST.1 = `61a04e7`** (D12). All ticked work verified present.
- **current wave:** Wave 0 + Lane C Studio COMPLETE. **Wave 1 in progress** — RU.1 done (Reply Master → 3 angle variants). Lane A prompt-spine (RU→N→ME→AI) is the open work.
- **next-up:** RU.2 (migration: reply `source` + radar `variants`/`model`/`reply_draft_id` columns — dep-free, high) then RU.3 (batch → 3 variants, needs RU.1✓+RU.2). Dep-free parallel starts: N.1 (niche schema, xhigh), AI.1 (OpenRouter, xhigh). **Prompt-surgery chain** next link is N.3 (needs N.1, after RU.1✓). **UI lane** blocked until UI.6.
...
- [x] RU.1 Reply prompt → 3 variants (xhigh) — 9c05464 2026-07-18
...
- **Stale comment left on purpose:** `src/x/db/schema.ts:319` still says "structured two-variant call" for the `variants` column — out of RU.1's edit-scope (schema.ts is RU.2/RU.3 hot-file territory). RU.2/RU.3 should fix it when they touch that file.
