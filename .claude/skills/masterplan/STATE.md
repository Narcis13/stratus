...
## WAVE 8 IS OPEN — 147/149. Next: RA.7, then RA.8 closes the wave.
...
- **last-commit:** **identity is the SUBJECT LINE — no sha is recorded (D97).** HEAD should read `fix(harvest): RA.6 passive timeline capture — the passiveHarvest toggle was off`, parent `64d22d9` (RA.5). **Step 0 is one command:** `git log -1 --format='%h %s'`.
