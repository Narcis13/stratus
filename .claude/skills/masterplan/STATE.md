...
## WAVE 9 IS OPEN — 153/157. Next up: OU.5 — one serial lane to the end (OU.5 → OU.6 → OU.7 → OU.8), and every remaining task touches `Outliers.tsx`.
...
- **last-commit:** **identity is the SUBJECT LINE — no sha is recorded (D97).** HEAD should read `feat(outliers): /x/searches CRUD + compile/run/defaults routes (OU.4)`, parent `8e97de7`. **Step 0 is one command:** `git log -1 --format='%h %s'`. **Expect ad-hoc commits between masterplan tasks** — four lanes ran outside the skill since RA.8, so a HEAD that doesn’t name a task is normal now; reconcile against this line, not against the whole log.
- **current state of the repo (as of OU.4, 2026-08-24 — recounted, not carried forward):** suite **2583** across **130** files; tables **44**; migrations through **`0031_sharp_screwball`** — **the journal is FREE from `0032`**; registry **16 groups / 69 knobs, 33 mirrored** and **17 prompt keys** (OU.3 added the six `x.outliers.*`, **none of them mirrored**, so only the knob and group totals moved); MCP **28 tools** (3 schema / 19 curated / 6 write) — **Wave 9 bumps none of them by decision**; smoke scripts **40**; extension tsconfig `include` shims **11**; panel tabs **14**; whole-repo lint **0 errors**. **`docs/settings-tab.md`’s count strings are CURRENT as of OU.3** (69 knobs / 16 groups / 33 mirrored / 17 prompts, plus the new **Outliers** table row and the `### The sixteen groups` heading). The old note here claimed SW.1 had left them stale; it had not (D205) — **re-read the strings before believing any staleness claim about them.** The four multi-file moves a new task will hit, each with the assertions that make them fail loudly: **a mirrored knob is SEVEN edits** — `registry.ts` (group array; order decides mirrored position), `registry.test.ts` **twice** (the group’s exact key list *and* the exact mirrored list), `docs/settings-tab.md`’s **three** count strings (prose, asserted by nothing) **plus its group table row**, and extension-side `ServerConfig` + `SERVER_DEFAULTS` + `readServerConfig`, pinned by `serverSettings.test.ts`’s exact `Object.keys` list AND its full-blob `toEqual` (D181d — the server half alone ships a knob that silently does nothing). **`x.outliers.*` is `scope: 'server'` throughout, which is why OU.3 owed the server three of those seven and not all seven — the first group in the repo to do so.** **A registry prompt key is FOUR** (`PROMPT_KEYS` + `PROMPT_SPECS`, the default exported from a **pure** module or the import cycles, `registry.test.ts`’s exact key list, and `docs/settings-tab.md`’s three prompt strings). **An MCP tool is SIX doc strings plus three asserted numbers**: `src/mcp.test.ts`’s exact **28**, `scripts/smoke-mcp.ts`’s expected-names list, and in `docs/s2-mcp-server.md` the prose total, the `## The tools` heading, the end-to-end verification line, the `### <tier> (N)` heading **of the tier you touched**, the intro’s prose enumeration of the write tools, and the §"Security & cost invariants" write-ceiling row — plus the counts in codemap §3.3’s `mcp.ts` row **and** §6. **A migration** never runs in two parallel sessions, ignores any number quoted in plan text, and is inspected for dropped seed INSERTs by `git status --porcelain src/db/migrations/` (**not `git diff --stat`** — the new `.sql` and its snapshot are UNTRACKED, so a diff shows only the `_journal.json` append and reads like the SQL never landed) plus a fresh `:memory:` boot counting `content_pillars` (3) — never by grepping for `INSERT INTO`, which `0000`’s `INSERT OR IGNORE` spellings make return 0 (D177a).
- **next-up:** **OU.5** (the Outliers tab — form, live preview, Copy + Open in X, saved list, the 12th shim, **high**) — deps OU.1 ✓ OU.4 ✓, so it is unblocked, and it is **the first task of three that all write `extension/src/sidepanel/Outliers.tsx` in strict series** (OU.5 → OU.6 → OU.7; the plan's `[parallel-ok]` tag on OU.7 is wrong — D195 in `plans/MASTERPLAN.md`). **6 files, 2 new**, and every one of them is extension-side, so this is the wave's FIRST task that owes `cd extension && bun run build` **plus** the extension's own `typecheck`, then `grep -cE '^\s*import[ {]' extension/dist/content.js` == 0. **Panel components carry no unit tests by convention (§5), so the server suite count must NOT move** — the verification is the build plus a grep over `dist/sidepanel.js` for `min_faves` / `-filter:replies` / `x.com/search`, proving Vite inlined the shared compiler rather than the shim resolving to nothing. The shim is the **12th** out-of-tree `include` in `extension/tsconfig.app.json` (copy `extension/src/replyBand.ts`'s exact 4-line shape); the wire types go in `extension/src/shared/types.ts` and must **re-export** `SearchQuery` from the shim, never redeclare it (D180a — a hand-mirrored type drifts). **What OU.4 actually shipped, which is what this task consumes** (D207–D209 below carry the reasoning): `GET /x/searches` returns `{searches, capture}` where every item is `{saved, compiled, url}` — **the same shape `GET /x/searches/:id` returns**, so Load is one function; `saved.query` is a COMPLETE `SearchQuery` with `sort` merged in from the column, so the form binds to it directly and posts it back whole (an explicit body `sort` is accepted but unnecessary — the query's own `sort` field moves the column); `GET /x/searches/defaults` returns `{query, ladder, problems}` and its `problems` may hold **one `warn` about `x.outliers.lang`**, which the tab should surface rather than swallow. **There is deliberately no `compile` method in `api.searches`** — the panel compiles locally through the shim on every keystroke with zero network traffic; `POST /x/searches/compile` exists for smoke/CLI callers only. `run(id)` is **fire-and-forget** (§7.8): a failed `last_run_at` stamp must never block the clipboard write or the tab move, and an unsaved ad-hoc query skips it entirely. Two things the task block forbids and a hurried session would do anyway: **do NOT touch `extension/src/content.ts`** (OU.7 owns its one-field change) and **do NOT build the ladder stepper or the channel/target prefills** (OU.6 owns them, and OU.5 is already at budget). Render `hasLinks` as a **three-state control (any / with / without)**, not a checkbox — the compiler emits both arms (D202). Standing gate reminders: `bun run test` (bare `bun test` targets the file DB and `inspect.test.ts` fails 2/2); biome forbids `delete process.env.X` / `: any` in `scripts/`, **non-null assertions (`x!`) anywhere**, a `let` assigned once, and a backtick string with no interpolation — it also sorts imports case-sensitively and rejects a `console.log` string-concat unless every operand interpolates (run `bunx biome check --write` on the changed files FIRST).
...
- **Wave 9 — Outliers: X advanced-search compiler, clipboard hand-off, saved hunts (4/8)**: plan
