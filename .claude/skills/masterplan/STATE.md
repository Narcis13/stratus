...
- **last-commit:** `2104f46` (ST.7). **Sha reconciliation (see D19):** at ST.7 start STATE said `b65266f` (ST.6) but HEAD was `00b9615` — `b65266f` was another amend-orphan (sha-stamp trap), real ST.6 commit is **`00b9615`**. Corrected in the ledger + codemap header + §11. Earlier same trap: ST.5 `bc3f1e8`→**`1f6b20f`**, ST.4 `c1b0314`→**`15f2563`**, ST.3 `292603d`→**`b82058b`**, UI.10 `50948c9`→**`c37d504`**, ST.2 `860725f`→**`dffd60a`**, UI.9 `8c89950`→**`f9ff346`**; rewritten `7e07dd5`/`40c718e`/`95b9fff` → actual Wave-0 **UI.1 = `226368c`, UI.8 = `1299e43`, ST.1 = `61a04e7`** (D12). All ticked work verified present; only recorded shas were ever stale, no status drift.
- **current wave:** Wave 0 foundations COMPLETE. Lane C Studio: ST.1/ST.2/ST.3/ST.4/ST.5/ST.6/**ST.7** done → **ST.8** (chart, dep ST.4+ST.5 ✓ — uses `api.metrics.account()` shipped in ST.5) eligible; then **ST.9** (docs+smoke+browser walk, dep all ST — ST.8 is the last blocker). Wave 1 (Lane A) still open.
- **next-up:** Lane C → **ST.8** (chart card — `api.metrics.account()` exists from ST.5, D18c; reuse `sparkline`/`sparklineCoords`, `metrics.bestTimes` for the heatmap; new `chartData.ts` + `chartData.test.ts`; register a 9th template — 1 registry row + 1 field section + 1 buildSpec case, no shell branches per D15). After ST.8 → **ST.9** (the whole-Studio browser walk that clears every "unverified in browser" flag ST.1–ST.8 accrued). All clean, no hot-file locks. Lane A → Wave 1 server (RU.1 / N.1 / AI.1 — all dep-free). **UI lane** still blocked until the Wave-1 settings-mirror chain (UI.6) lands.
...
- [x] ST.7 Thread cover + list card (high) — 2104f46 2026-07-18
