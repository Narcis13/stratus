...
- **last-commit:** `47b10d2` (ST.9). **Sha reconciliation (see D22):** at ST.9 start STATE said `d38bbc5` (ST.8) but HEAD was `289ec2c` — `d38bbc5` was another amend-orphan (sha-stamp trap), real ST.8 commit is **`289ec2c`**. Corrected in the ledger + codemap header + §11. Earlier same trap: ST.7 `2104f46`→**`acd95ec`**, ST.6 `b65266f`→**`00b9615`**, ST.5 `bc3f1e8`→**`1f6b20f`**, ST.4 `c1b0314`→**`15f2563`**, ST.3 `292603d`→**`b82058b`**, UI.10 `50948c9`→**`c37d504`**, ST.2 `860725f`→**`dffd60a`**, UI.9 `8c89950`→**`f9ff346`**; rewritten `7e07dd5`/`40c718e`/`95b9fff` → actual Wave-0 **UI.1 = `226368c`, UI.8 = `1299e43`, ST.1 = `61a04e7`** (D12). All ticked work verified present; only recorded shas were ever stale, no status drift.
- **current wave:** Wave 0 foundations COMPLETE. **Lane C Studio (ST.1–ST.9) COMPLETE** — the whole S5 gallery is browser-verified (D22). Wave 1 (Lane A) is the only open work.
- **next-up:** Lane A → **Wave 1 server**: RU.1 (reply prompt → 3 variants) / N.1 (niche schema) / AI.1 (OpenRouter client) — all dep-free, pick by lane. **UI lane** stays blocked until the Wave-1 settings-mirror chain (UI.6) lands. No Studio work remains.
...
- [x] ST.9 Studio docs+smoke (high) — 47b10d2 2026-07-18
