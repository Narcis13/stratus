...
## WAVE 8 IS OPEN — 146/149. Next: RA.7 (RA.6 floats and can run beside anything).
...
- **last-commit:** **identity is the SUBJECT LINE — no sha is recorded (D97).** HEAD should read `feat(extension): RA.5 Fetch drafts action on the Radar tab`, parent `e9e9f76` (RA.4). **Step 0 is one command:** `git log -1 --format='%h %s'`.
...
- **Wave 8 — Radar access from a Claude Code session (5/8)**, plan `plans/2026-08-17-radar-access.md`, opened by `d968fe9` (`wave 8` — plan + MASTERPLAN registration only, no code, no STATE.md; RA.1 owed the re-opening and did it). **$0 lane.** Took migration `0029_blushing_expediter` (`radar_sightings`).
