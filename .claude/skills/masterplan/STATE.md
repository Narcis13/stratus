...
- **last-commit:** 860725f (ST.2). **Sha reconciliation (see D14a / D15):** UI.10's recorded `50948c9` was another amend-orphan (sha-stamp trap) — the real UI.10 commit is **`c37d504`** (confirmed via git log at ST.2 start; HEAD was `c37d504`, STATE said `50948c9`); corrected in the ledger + codemap §11. Earlier: UI.9's `8c89950`→**`f9ff346`**; the rewritten `7e07dd5`/`40c718e`/`95b9fff` → actual Wave-0 **UI.1 = `226368c`, UI.8 = `1299e43`, ST.1 = `61a04e7`** (D12). All ticked work verified present; only recorded shas were ever stale, no status drift.
- **current wave:** Wave 0 foundations COMPLETE. Lane C Studio: ST.2 done → ST.3/ST.4 now eligible (both dep on ST.1+ST.2). Wave 1 (Lane A) still open.
- **next-up:** Lane C → ST.3 (Cloud mascot, dep ST.1+ST.2 ✓) or ST.4 (patterns+presets, dep ST.1+ST.2 ✓) — both eligible now. Lane A → Wave 1 server (RU.1 / N.1 / AI.1 — all dep-free). **UI lane** still blocked until the Wave-1 settings-mirror chain (UI.6) lands.
...
- [x] ST.2 Studio shell refactor (high) — 860725f 2026-07-18
