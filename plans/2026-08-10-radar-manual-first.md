...
**Done when:**
- [ ] With no `radar:sweep` key, scrolling x.com/home adds zero Radar rows; the border/dim still renders and `/x/harvest/passive` still receives batches
- [ ] With the key set by hand to an active session, only tweets satisfying the filters enter, carrying `'sweep'` (or `hot`/`warm` when the classifier agreed)
- [ ] An expired session captures nothing without any timer having fired
- [ ] `bun test` + `bun run typecheck` + `bun run lint` green; `cd extension && bun run build` green
- [ ] Committed: `feat(radar): RS.3 manual by default — capture only while the sweep is armed`
...
**Done when:**
- [ ] Start → the row reads `Sweeping · Nm left` and the count decrements; Stop → back to manual; Start again works in the same panel session
- [ ] Letting the timer run out flips the row to the ended notice without a reload
- [ ] The gear edits all eleven knobs and a refused value shows its code on the row (the `useSettingsEditor` path)
- [ ] `bun test` + `bun run typecheck` + `bun run lint` green; `cd extension && bun run build` green
- [ ] Committed: `feat(radar): RS.4 start/stop the sweep, with the filters one click away`
