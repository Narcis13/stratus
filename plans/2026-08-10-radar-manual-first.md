...
- [x] `docs/PHASE-HISTORY.md` — the RS phase entry (what shipped, 2026-08-10, $0, the three "looks like a bug" consequences from the Design section).
- [x] `docs/radar-tab.md` — manual as the default state, Start/Stop, the eleven knobs and what each admits, the chip, the lit ⊕, and the sentence that keeps the band group legible: the twelve `x.band.*` thresholds still draw the border and still gate `/replies/generate`; they no longer decide what enters the queue.
- [x] `docs/settings-tab.md` — the new `sweep` group in Tuning.
- [x] `CIRCLES-PLAN.md` — status line (the Radar is C0/C4 territory).
- [x] `CLAUDE.md` — **only if a guardrail changed.** It did not; leave it. (State that explicitly in the commit body so the next session does not re-check.)
...
- [ ] **Browser pass, by hand, and it is the only proof of the DOM half** (`content.ts` is untested by convention): on a real x.com/home scroll — manual captures nothing; Start captures only what the filters allow; `verifiedOnly: true` admits a verified author's post and refuses an unverified one *in the same scroll* (this is the selector check — if it admits nothing at all, `readVerified` has drifted); the chip appears and stops the sweep; a queued tweet's ⊕ stays lit after scroll-back; expiry ends capture. Record the result in the phase entry. **← UNPAID. Parked as `VERIFY-DEBT (0u)` with the full walk written out; it needs a human at a real logged-in timeline with the extension reloaded, which is the one step of this plan a session cannot do for you.**

**Shipped note (RS.6, the closer):**
1. **The smoke's step (b) is the only end-to-end claim available for this group**, and it is why it PATCHes rather than only reading defaults: `x.sweep.*` has **no server consumer at all**, so "the registry holds eleven correct knobs" and "the page will receive them" are different facts, and only `values?scope=mirrored` carrying an *edited* value proves the second. Copy this shape for any future mirrored-only group.
2. **Snapshot/restore generalized from one row to a group.** `smoke-humanizer.ts` protects a single `app_settings` row with a string sentinel inside a written pool; here the sentinel is a *number no operator would type* (`987654` on `x.sweep.maxViews`), and finding it at entry means the whole group's baseline is treated as ABSENT rather than adopted. A numeric knob has no natural sentinel — pick an in-range value that is obviously synthetic, and say so where it is defined.
3. **The registry counts were recounted from `SETTINGS_REGISTRY`, not carried forward** (17 groups / 79 knobs / 44 mirrored — they happen to match RS.1's note, which is the point of recounting rather than a reason to skip it). Same for the tsconfig shims: the `include` array holds **eleven**, and §5's sentence has now been wrong twice, so it was rewritten to say so.
4. **One named test gap, recorded in codemap §9 rather than quietly closed:** `ACCEPTED_BATCH_BANDS` in `routes/replies.ts` — the runtime `Set` that would have 400'd every swept row — has no `'sweep'` assertion. `src/x/routes/replies.test.ts` exists and is where it belongs; adding it was outside this task's scope, and naming it beats leaving it invisible.
5. **`CLAUDE.md` was not touched, deliberately.** No guardrail changed: §7.4, §7.11, §7.19, §7.24, §7.26, §7.27 and §7.33 were each *applied* by this lane, none amended. Do not re-check this.
