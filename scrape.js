...
(() => {
  ...
  const PRESETS = {
    slow: {
      flickMin: 0.25,
      flickMax: 0.5, // fraction of a viewport per flick
      pauseMin: 2200,
      pauseMax: 4200, // normal "reading" pause (ms)
      longChance: 0.2,
      longMin: 5000,
      longMax: 9000, // occasional long break
      backChance: 0.12,
      backMin: 0.1,
      backMax: 0.3, // occasional scroll-up
      loadMin: 2200,
      loadMax: 3800, // wait at bottom for new tweets
  ...
  function idFrom(art) {
    const a = Array.from(art.querySelectorAll('a[href*="/status/"]')).find((x) =>
  ...
  function repliesCSV(store) {
    ...
    const rows = Object.values(store).sort((a, b) =>
      (b.r_time || '').localeCompare(a.r_time || ''),
    );
  ...
  async function run(mode, opts) {
    ...
    if (!profile) {
      console.error('[xHarvest] Could not read a handle from the URL.');
      return;
    }

    const onReplies = /\/with_replies\b/.test(location.pathname);
    if (mode === 'replies' && !onReplies)
      console.warn('[xHarvest] Not on /with_replies — navigate there for reply pairs.');
    if (mode === 'posts' && onReplies)
      console.warn('[xHarvest] You are on /with_replies. For pure posts use the main profile tab.');
    if (document.visibilityState !== 'visible')
      console.warn(
        '[xHarvest] Tab looks backgrounded — bring it to the foreground or X will not load more.',
      );
    ...
    const oldestOf = (s) =>
      Object.values(s).reduce((min, r) => {
        const t = mode === 'replies' ? r.r_time : r.time;
    ...
    console.log(
      '[xHarvest] ' +
        ...
        ' — pace "' +
        (opts.pace || 'human') +
        '". Keep this tab visible; this is intentionally slow.',
    );
    ...
    while (steps < HARD_STEP_CAP) {
      ...
      if (count >= max) {
        console.log('[xHarvest] Reached max=' + max + '.');
        break;
      }
      if (untilDate && oldest && oldest < untilDate) {
        console.log('[xHarvest] Reached date cutoff ' + opts.untilDate + '.');
        break;
      }
      ...
      if (atBottom) {
        if (se.scrollHeight === lastH) {
          ...
          if (stable >= cfg.stableNeeded) {
            console.log('[xHarvest] Bottom reached, no new content.');
            break;
          }
        } else stable = 0;
    ...
    const times = rows
      .map((r) => (mode === 'replies' ? r.r_time : r.time))
      .filter(Boolean)
      .sort();
    console.log(
      '[xHarvest] DONE. ' +
  ...
  window.xHarvest = {
    ...
    redownload: () => {
      const L = window.__xHarvestLast;
      if (L) download(L.csv, L.name);
    },
  };
  console.log('%c[xHarvest] loaded (human-paced).', 'color:#1d9bf0;font-weight:bold');
  console.log(
    'Run:  xHarvest.posts()   or   xHarvest.replies()   ·   options: { pace:"slow|human|fast", max, untilDate }',
  );
})();
