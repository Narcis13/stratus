...
The registry holds **69 knobs in 16 groups** and this tab renders **all of them**. It shrank twice before it grew: the twelve `x.band.*` thresholds that used to make it 79-in-17 were deleted on 2026-08-11 (see *[The reply band is gone](#the-reply-band-is-gone)* below), and on **2026-08-12** the whole **Mentions** group and four of the five **Workers** knobs went with the billed X reads they configured — there is no daily metrics pass and no mention pull left to schedule. The newest group is **Outliers** (2026-08-24), the six defaults a fresh hunt on the [Outliers](./outliers-tab.md) tab opens with. This is the map of what's here — what each group decides, and where you see it change.
...
| **Follow-ups** | 9 | The follow-up queue's windows: chain freshness, DM readiness, neglected target/ally days, momentum %, re-up age range **and view floor**, unacknowledged-fan days. | Do next, Targets, Top fans, the dossier queue. |
...
| **Sweep** | 13 | **What an armed sweep is allowed to put into the Radar queue**: min/max impressions, min/max likes, min/max replies, a max tweet age, a **photos & videos** gate (`any` / `with` / `without` — ships `any`), a **skip promoted posts** switch (**on**), a verified-authors-only switch, two bypass switches (camped cannon accounts — **on**; circle accounts — **off**), and how long one sweep runs before it auto-stops (30 min). All thirteen are **mirrored** — the page is what enforces them. | The **[Radar](./radar-tab.md#filling-the-queue)** tab's queue, while a sweep is armed. The same thirteen rows are editable from the ⚙ next to **Start sweep** without leaving the Radar. |
...
| **Outliers** | 6 | **What a fresh outlier hunt opens with**: the three engagement floors (min likes — ships **400**; min reposts and min replies — both ship **0**, meaning "no operator"), how many days back it looks (**30**), a language code (**blank** = no `lang:` filter), and which x.com results tab **Open in X** lands on (**top**, because a hunt wants best-performing rather than newest). None is mirrored — these are *defaults handed to the form*, not rules anything enforces, and a saved hunt keeps its own copy. | The **[Outliers](./outliers-tab.md)** tab's form, when you start a new hunt — the tab also carries its own gear over these same six. Changing one here never touches a hunt you already saved, and never touches the form you have open; it changes what **New hunt** starts from. |
| **Workers** | 1 | The publisher tick — how often the scheduler looks for a post that is due. | Background only, and a `restart` knob. The other four (the daily-metrics hour, the winner re-read floor/cap, **Discovery skips replies**) went on 2026-08-12 with the billed reads they scheduled: publishing is the only worker left that talks to X. |
...
Thirty-three of the 69 are also **mirrored** to the extension (Identity, all of Doctrine, **Sweep** and Cannon, most of Display), which is what lets the side panel and the injected on-page UI show the same numbers the server decides with. The rest are server-only, because the panel already receives their effect rather than the number. The Sweep group is the one whose mirroring is not a convenience but the mechanism: no server route reads those thirteen settings to decide a capture — the content script does, off the mirrored blob.
...
- **Why.** After the Radar went manual-first, those twelve stopped deciding what enters the queue — the **Sweep** filters do. Hiding them wasn't enough: the classifier was still running behind your back, painting a green/amber rail down the timeline and refusing the occasional Reply Master draft on numbers you had no way to change. A second admission rule you can't tune is worse than no second rule. There is now exactly **one** place that decides what a tweet qualifies for: the **⚙ next to Start sweep** on the Radar (the same thirteen knobs also appear as the **Sweep** group above).
...
- **Restore Default Prompts:** the big button (with a confirm dialog) that deletes **every** override at once — all 17 prompts snap back to their shipped defaults and the "customized" chips disappear. This is the recovery button if an edit made your drafts worse.
