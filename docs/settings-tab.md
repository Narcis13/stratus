...
The registry holds **63 knobs in 15 groups** and this tab renders **all of them**. It has shrunk twice: the twelve `x.band.*` thresholds that used to make it 79-in-17 were deleted on 2026-08-11 (see *[The reply band is gone](#the-reply-band-is-gone)* below), and on **2026-08-12** the whole **Mentions** group and four of the five **Workers** knobs went with the billed X reads they configured — there is no daily metrics pass and no mention pull left to schedule. This is the map of what's here — what each group decides, and where you see it change.
