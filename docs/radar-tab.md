...
- An **E chip** — `E 52 · strong shape` — on any row whose sighting carried a view count. Same 26 published X ranking weights as the Composer's **C** pill (see [Composer → the ranker score](./composer-tab.md#the-ranker-score-c)), applied backwards: over what this post has *actually* done — its likes and replies against its views — rather than over what a draft might do. 50 is a post doing your feed's median rates; the band is cut at 46/51, the measured quartiles of the 3,567 sightings you've already harvested, so it says where this post sits among them. Three things it deliberately doesn't do: it **scores the post, never your reply** (X drops out-of-network replies from For You outright, so a reply's own ranker score would be close to meaningless — the tooltip says so); a sighting carries no repost count, so that head is **left out rather than counted as zero**; and a sighting with no view count shows **no chip at all**, because `E 0` would read as "this post is dead" instead of "we didn't look". Hover it for the heads it used, the view count it rated against, and the same *context, not advice* disclaimer. Nothing sorts or filters on it.
