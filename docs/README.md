...
| **[Radar](./radar-tab.md)** | Your reply queue — tweets your sweep filters caught live while you browse X ($0), ranked; one Grok call drafts three angles for the whole queue, and clicking the angle you want copies it and opens the tweet. |
...
- **Sweep** — a bounded session during which the Radar fills itself, admitting tweets that clear filters *you* state (impressions, likes, replies, age, verified). It is the only rule that decides what a tweet qualifies for; a reply-band classifier used to compete with it and was deleted on 2026-08-11. See **[Radar](./radar-tab.md)**.
