...
> **Post-plan, same Radar lane: the queue became one queue (RQ, 2026-08-18, `plans/2026-08-18-radar-live-queue.md`, $0).** A dismissal used to live only in `chrome.storage.local`, so the panel read **Queue (35)** while `/radar-analyst` read **55** and a drafting session re-surfaced tweets the operator had already waved off. `radar_sightings` gained `dismissed_at` (migration `0030`), the panel's ✕/**Clear**/consume paths mirror onto it through one best-effort `PATCH /x/radar/sightings` beside the existing `markDraftsOnServer` (§7.8 — it never fails or delays the buffer write), and **`GET /x/radar/sightings?queue=true`** (plus `x_radar`'s `queue` input) rebuilds the panel's own predicate server-side: seen inside 24 h, not dismissed, not worked. Ratchet rules match the panel exactly — a re-dismiss updates nothing, a plain re-sighting does not resurrect a tombstoned row, and only a `manual`-band ⊕ re-pin clears it. C0's persistence contract is untouched (`radar_drafts` gained no column) and no panel UI moved: the `Queue (N)` badge the operator already reads IS the contract. See `docs/radar-tab.md`.
